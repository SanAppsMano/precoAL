  ...
  async function runSearch(code, latitude, longitude, raio){
    try{
      ...
      if(!resp.ok || data.length===0){
        const municipioNome = citySel.options[citySel.selectedIndex].text;
        resultDiv.innerHTML = `
          <p class="error">
            Nenhum resultado encontrado para os últimos <strong>3 dias</strong><br>
            em <strong>${municipioNome}</strong>, raio de <strong>${raio} km</strong>.
          </p>`;
        return;
      }
      ...
      const item = { code, city: citySel.value, productName: data[0].dscProduto||"—", thumbnail: data[0].codGetin?`https://cdn-cosmos.bluesoft.com.br/products/${data[0].codGetin}`:"https://via.placeholder.com/100", minEntry: minE, maxEntry: maxE, total: data.length };
      renderResult(item);
