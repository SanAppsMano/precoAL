window.addEventListener('DOMContentLoaded', () => {
  ...
  // History management (cached render)
  let historyData = [];
  ...
  function renderResult({ productName, thumbnail, minEntry, maxEntry, total, code, city }) {
    ...
    // Store code/city for potential updates
  ...
  btnSearch.addEventListener('click', () => {
    // If update-state, clear it to default
    ...
    if(document.querySelector('input[name="loc"]:checked').value==='gps'){
      navigator.geolocation.getCurrentPosition(pos=>runSearch(code,pos.coords.latitude,pos.coords.longitude,raio),()=>{alert("Não foi possível obter sua localização.");loadingDiv.classList.remove('show');});
    } else {
  ...
  async function runSearch(code, latitude, longitude, raio){
    try{const resp=await fetch(FN_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({codigoDeBarras:code,dias:3,latitude,longitude,raio})});const text=await resp.text();if(!text.trim()||text.trim().startsWith("<")){resultDiv.innerHTML=`<p class="error">Erro ${resp.status}</p>`;return;}const data=JSON.parse(text);if(!resp.ok||!data.length){resultDiv.innerHTML=`<p class="error">Nenhum resultado.</p>`;return;}const minE=data.reduce((a,b)=>b.valMinimoVendido<a.valMinimoVendido?b:a,data[0]);const maxE=data.reduce((a,b)=>b.valMaximoVendido>a.valMaximoVendido?b:a,data[0]);const item={code,city:citySel.value,productName:data[0].dscProduto||"—",thumbnail:data[0].codGetin?`https://cdn-cosmos.bluesoft.com.br/products/${data[0].codGetin}`:"https://via.placeholder.com/100",minEntry:minE,maxEntry:maxE,total:data.length};renderResult(item);loadHistory();addToHistory(item);}catch(err){console.error(err);resultDiv.innerHTML=`<p class="error">Falha de rede: ${err.message}</p>`;}finally{loadingDiv.classList.remove('show');}}
