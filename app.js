window.addEventListener('DOMContentLoaded', () => {
  ...
  // Ensure loading is hidden initially via hidden attribute
  ...
  const quaggaConfig = {
    ...
    decoder: { readers: ["ean_reader"] }, locate: true
  };
  btnScan.addEventListener('click', () => {
    if (!scanning) {
      Quagga.init(quaggaConfig, err => { if (err) { alert("Não foi possível acessar a câmera."); return; } Quagga.start(); scanning = true; btnScan.textContent = "Parar Scanner"; });
      Quagga.onDetected(d => { if (d.codeResult?.code) { barcodeIn.value = d.codeResult.code; Quagga.stop(); scanning = false; btnScan.textContent = "Iniciar Scanner"; } });
    } else { Quagga.stop(); scanning = false; btnScan.textContent = "Iniciar Scanner"; }
  });

  // History management
  let historyData = [];
  function loadHistory() {
    const raw = localStorage.getItem("searchHistory"); historyData = raw ? JSON.parse(raw) : [];
  }
  function saveHistory() {
    localStorage.setItem("searchHistory", JSON.stringify(historyData));
  }
  function renderHistory() {
    ulHistory.innerHTML = historyData.map((it,i) => `<li data-index="${i}"><img src="${it.thumbnail||'https://via.placeholder.com/60'}" alt="${it.productName}"/></li>`).join('');
  }
  ...
  ulHistory.addEventListener('click', e => { const li = e.target.closest('li'); if (!li) return; const item = historyData[li.dataset.index]; barcodeIn.value = item.code; if (document.querySelector('input[name="loc"]:checked').value==='city') citySel.value = item.city; btnSearch.click(); });
  ...
  function renderResult({ productName, thumbnail, minEntry, maxEntry, total }) {
    ...
    resultDiv.innerHTML = html; resultDiv.scrollIntoView({behavior:'smooth'});
  }
  ...
  btnSearch.addEventListener('click', () => {
    ...
    if(document.querySelector('input[name="loc"]:checked').value==='gps'){
      navigator.geolocation.getCurrentPosition(pos=>runSearch(code,pos.coords.latitude,pos.coords.longitude,raio),()=>{alert("Não foi possível obter sua localização.");loadingDiv.hidden=true;});
    } else {
  ...
  async function runSearch(code, latitude, longitude, raio){
    try{
      const resp = await fetch(FN_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({codigoDeBarras:code,dias:3,latitude,longitude,raio})});
      const text = await resp.text();
      if(!text.trim()||text.trim().startsWith("<")){resultDiv.innerHTML=`<p class="error">Erro ${resp.status}</p>`;return;}
      const data = JSON.parse(text);
      if(!resp.ok||!data.length){resultDiv.innerHTML=`<p class="error">Nenhum resultado.</p>`;return;}
      ...
      renderResult({code,city:citySel.value,productName:data[0].dscProduto||"—",thumbnail:data[0].codGetin?`https://cdn-cosmos.bluesoft.com.br/products/${data[0].codGetin}`:"https://via.placeholder.com/100",minEntry:minE,maxEntry:maxE,total:data.length});
      loadHistory();addToHistory({code,city:citySel.value,productName:data[0].dscProduto||"—",thumbnail:data[0].codGetin?`https://cdn-cosmos.bluesoft.com.br/products/${data[0].codGetin}`:"https://via.placeholder.com/100",minEntry:minE,maxEntry:maxE,total:data.length});
    }catch(err){console.error(err);resultDiv.innerHTML=`<p class="error">Falha de rede: ${err.message}</p>`;}finally{loadingDiv.hidden=true;}}
